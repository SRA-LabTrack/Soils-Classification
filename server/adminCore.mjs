...
async function createSensor(payload) {
  ...
  const previous=await getRow(tables.sensors,sensorId);
  const stamp=now();
  const readingData={
  ...
  let stationRow;
  let schemaWarning='';
  try{stationRow=await upsertRow(tables.sensors, sensorId, stationData, permission);}
  catch(error){
    ...
    stationRow=await upsertRow(tables.sensors,sensorId,legacyStationData,permission);
...
async function createPlot(payload) {
  ...
  let plotRow;
  let schemaWarning='';
  try{plotRow=await upsertRow(tables.plots,plotId,plotData,permission);}
  catch(error){
    ...
    plotRow=await upsertRow(tables.plots,plotId,legacyPlotData,permission);
...
async function createDroneMapping(payload) {
  ...
  let createdRow;
  let schemaWarning='';
  try{createdRow=await upsertRow(tables.drone,id,data,perms(farm.farmer_id));}
  catch(err){
    ...
    createdRow=await upsertRow(tables.drone,id,legacyData,perms(farm.farmer_id));
...
async function updateSpatialRequestRow(rowId,data,farmerId){
  try{return await updateRow(tables.requests,rowId,data,requestPerms(farmerId));}
  catch(error){
    if(!isOptionalSchemaError(error))throw error;
    const minimal={status:data.status};
    if(data.resolved_entity_id!==undefined)minimal.resolved_entity_id=data.resolved_entity_id;
    try{return await updateRow(tables.requests,rowId,minimal,requestPerms(farmerId));}
    catch(second){return updateRow(tables.requests,rowId,{status:data.status},requestPerms(farmerId));}
  }
}
...
async function listSpatialRequestsForFarmer(farmerId,{pendingOnly=false}={}){
  ...
  // Avoid requiring a farmer_id + created_at composite index just to display
  // a small request list. Filter by indexed equality, then order in memory.
  const queries=[Query.equal('farmer_id',[farmerId])];
  if(pendingOnly)queries.push(Query.equal('status',['pending']));
  const result=await listRows(tables.requests,queries,120);
  return uniqueRows(result?.rows||[]).map(normalizeSpatialRequest).sort((a,b)=>rowStamp(b)-rowStamp(a));
}
...
async function reviewSpatialRequest(payload){
  await requestTableAvailable();
  const requestId=String(payload.request_id||'').trim();const decision=String(payload.decision||'').toLowerCase();
  ...
  const raw=await getRow(tables.requests,requestId);if(!raw)throw Object.assign(new Error('This Farmer request no longer exists.'),{status:404});
  const request=normalizeSpatialRequest(raw);if(request.status!=='pending')return {request,alreadyReviewed:true};
  const farm=await getRow(tables.farms,request.farm_id);if(!farm)throw Object.assign(new Error('The requested farm no longer exists.'),{status:404});
  let entity=null,entityType=request.request_type;
  if(decision==='approve'){
    const token=`approval:${requestId}`;
    if(entityType==='sensor'){
      entity=await createSensor({farm_id:request.farm_id,mutation_id:token,sensor_code:request.title||'Requested Sensor',latitude:request.latitude,longitude:request.longitude,coverage_m:request.coverage_m||50,orientation_deg:request.orientation_deg||0,status:'Planned',nitrogen:0,phosphorus:0,potassium:0,ph:0,organic_matter:0,moisture:0});
    }else if(entityType==='plot'){
      entity=await createPlot({farm_id:request.farm_id,mutation_id:token,plot_code:request.title||'Requested Soil Plot',geojson:request.boundary,latitude:request.latitude,longitude:request.longitude,coverage_m:70,classification:'Pending',nitrogen:0,phosphorus:0,potassium:0,ph:0,organic_matter:0,notes:`Approved Farmer request${request.notes?`: ${request.notes}`:''}`});
    }else{
      const stats=polygonAreaStats(request.boundary);
      entity=await createDroneMapping({farm_id:request.farm_id,mutation_id:token,name:request.title||'Requested Drone Mapping',geojson:request.boundary,...stats,status:'Planned',classification:'Unclassified',nitrogen:0,phosphorus:0,potassium:0,ph:0,organic_matter:0,moisture:0,notes:`Approved Farmer request${request.notes?`: ${request.notes}`:''}`,captured_at:now()});
    }
  }
  const resolvedId=decision==='approve'?(entity?.sensorId||entity?.plotId||entity?.droneId||entity?.row?.id||''):'';
  const status=decision==='approve'?'approved':'rejected';const note=String(payload.note||'').trim();
  const updated=await updateSpatialRequestRow(requestId,{status,reviewed_at:now(),reviewed_by:'admin',resolution_note:note,resolved_entity_id:resolvedId},request.farmer_id);
  const label=entityType==='sensor'?'Sensor':entityType==='plot'?'Soil Plot':'Drone Mapping';
  await createSupportMessage({farmerId:request.farmer_id,farmId:request.farm_id,senderId:'system',senderRole:'system',senderName:'SOILS Support',message:decision==='approve'?`Your ${label} map request "${request.title}" was approved by the administrator.`:`Your ${label} map request "${request.title}" was not approved.${note?` ${note}`:''}`,readByAdmin:true,readByFarmer:false,requestId,requestType:entityType});
  const normalized=normalizeSpatialRequest({...raw,...updated,status,reviewed_at:updated?.reviewed_at||now(),resolution_note:note,resolved_entity_id:resolvedId});
  await writeSpatialChange({farmId:request.farm_id,type:'request',entityId:requestId,action:'upsert',row:normalized,logicalKey:request.title}).catch(()=>{});
  invalidateWorkspaceSnapshots(request.farm_id);
  return {request:normalized,decision,entityType,entity:entity?.row||null,entityResult:entity||null};
}

async function listSupportMessagesForFarmer(farmerId){
  await supportTableAvailable();
  // Keep chat payloads bounded. The UI only needs the recent conversation and
  // older rows remain in Appwrite if historical export is ever needed.
  const result=await listRows(tables.support,[Query.equal('farmer_id',[farmerId]),Query.orderDesc('created_at')],120);
  return uniqueRows(result?.rows||[]).map(row=>({...row,id:row.$id||row.id})).reverse();
}
...
async function getSupportThreads(){
  ...
  const requests=uniqueRows(requestResult?.rows||[]).map(row=>{
    const request=normalizeSpatialRequest(row);const farm=farmById.get(request?.farm_id)||farmByFarmer.get(request?.farmer_id)||null;
    return {...request,farmer_name:farm?.farmer_name||'Farmer',farm_name:farm?.name||''};
  });
...
async function farmerListSupportMessages(jwt){
  const user=await verifyFarmerJWT(jwt);
  const messages=await listSupportMessagesForFarmer(user.$id);
  // Mark Admin replies as seen when the Farmer fetches the conversation.
