...
export default function RecordPreview({sensor,plot,drone,farmName,editable=false,onEdit,onRotate,onDelete,onClose,busy=false}){
  ...
  return <aside className={`record-preview-card record-preview-${kind}`}>
    ...
    <div className="record-preview-status"><StatusPill value={status}/>{row.pending_sync&&<span className="pending-record-pill">Pending sync</span>}<span>{kind==='sensor'?`${row.coverage_m||50}m × ${row.coverage_m||50}m coverage`:kind==='plot'?'Laboratory observation area':`${Number(row.area_hectares||0).toFixed(2)} ha mapped`}</span></div>
