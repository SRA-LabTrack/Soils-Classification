...
function MapSectionCycleControl({farms=[],sensors=[],plots=[],drone=[],section='farms',onSectionChange,onFocus,onDeletePlotSource=null,collapsed=false,onCollapsedChange}){
  ...
  const sources=(targets[section]||[]).map((item,index)=>({id:item.id,title:item.title,meta:item.meta,value:item.value,onClick:()=>{cycleRef.current[section]=index;onFocus?.(item,{section,index,total:(targets[section]||[]).length});},onDelete:section==='plots'&&onDeletePlotSource?()=>onDeletePlotSource(item.row):undefined,deleteTitle:section==='plots'?'Remove this Soil Plot source':undefined}));
...
function MapWorkspace({
  ...
  drawMode, drawPoints, onMapPoint, drawCoverageM, drawOrientation, onDrawOrientation, toolbar, sectionControl=null, showMapPopups=true, preview=null, fitRequestKey=0, fitBoundaryIndex=null, dataRevision=0, modeHint=null, embedded=false,
}) {
...
export default function DashboardPage({ mode='admin' }) {
  ...
  async function focusMapTarget(target){
  ...
  async function reviewActiveSpatialRequest(decision){
    ...
    try{
      const result=await adminAction('reviewSpatialRequest',{request_id:activeSpatialRequest.id,decision},{force:true});
      const nextRequest=result?.request||{...activeSpatialRequest,status:decision==='approve'?'approved':'rejected'};
      setActiveSpatialRequest(nextRequest);
      if(decision==='approve'&&result?.entity){
        ...
        mergeConfirmedRow(action,{...result.entity,id:result.entity.id||result.entity.$id},nextRequest.farm_id);
      }
      setNotice(decision==='approve'?'Farmer request approved. The requested location is now a published Admin map record.':'Farmer request rejected. No map record was created, and the Farmer was notified in Support Chat.');
    }catch(error){setNotice(error.message||'The request could not be reviewed.');}
...
function FarmDetail({farm,sensors,plots,drone,requests=[],userMode,overview,admin,toolbar,drawProps,selectedSensorId,selectedPlotId,selectedDroneId,focusTarget,onSensor,onPlot,onDrone,onFocusMap,selectedSensor,selectedPlot,selectedDrone,onSaveSensor,onSaveDrone,onDeleteSensor,onDeletePlot,onDeleteDrone,onDeleteFarmer,busy,mapRevision,preview,fitRequestKey=0,fitBoundaryIndex=0}){
  ...
  const sensorSources=(key,digits=1,suffix='')=>sensors.map(s=>({id:s.id,title:s.sensor_code,meta:`Sensor pin • ${coordText(s)}`,value:`${number(s[key],digits)}${suffix}`,onClick:()=>onSensor?.(s),active:selectedSensorId===s.id}));
  const sensorCountSources=sensors.map(s=>({id:s.id,title:s.sensor_code,meta:`${s.status||'Online'} • ${coordText(s)}`,value:`${s.coverage_m||50}m`,onClick:()=>onSensor?.(s),active:selectedSensorId===s.id}));
  const plotSources=plots.map(p=>({id:p.id,title:p.plot_code,meta:`${p.classification||'Pending'} • ${niceDate(p.analyzed_at)}`,value:`pH ${number(p.ph,2)}`,onClick:()=>onPlot?.(p),onDelete:admin?()=>onDeletePlot?.(p):undefined,deleteTitle:'Remove this Soil Plot source',active:selectedPlotId===p.id}));
  const droneSources=drone.map(d=>({id:d.id,title:d.name,meta:`${d.classification||d.status||'Mapped'} • ${niceDate(d.captured_at)}`,value:`${number(d.area_hectares,2)} ha`,onClick:()=>onDrone?.(d),active:selectedDroneId===d.id}));
  const boundaries=farmBoundaries(farm);
...
function Statistics({farms,sensors,plots,drone=[],activeFarmId,onSensor,onPlot,onDrone,selectedSensorId,selectedPlotId,selectedDroneId,focusTarget,preview,mapRevision=0}){
  ...
  const nutrientSourceItems=(metricId,key,digits,suffix='')=>farmSensors.map(s=>({id:s.id,title:s.sensor_code,meta:`Sensor pin • ${coordText(s)}`,value:`${number(s[key],digits)}${suffix}`,active:selectedSensorId===s.id,onClick:()=>{setSection(metricId);onSensor?.(s)}}));
  const activeSensorSources=farmSensors.map(s=>({id:s.id,title:s.sensor_code,meta:`${s.status||'Online'} • ${coordText(s)}`,value:`${s.coverage_m||50}m`,active:selectedSensorId===s.id,onClick:()=>{setSection('sensors');onSensor?.(s)}}));
  const plotSourceItems=farmPlots.map(p=>({id:p.id,title:p.plot_code,meta:`${p.classification||'Pending'} • ${niceDate(p.analyzed_at)}`,value:`pH ${number(p.ph,2)}`,active:selectedPlotId===p.id,onClick:()=>{setSection('plots');onPlot?.(p)}}));
  const droneSourceItems=farmDrone.map(d=>({id:d.id,title:d.name,meta:`${d.classification||d.status||'Mapped'} • ${niceDate(d.captured_at)}`,value:`${number(d.area_hectares,2)} ha`,active:selectedDroneId===d.id,onClick:()=>{setSection('drone');onDrone?.(d)}}));
  const boundarySource=farmBoundaries(farm).map((poly,index)=>({id:`${farm.id}-boundary-${index}`,title:`Farm Boundary ${index+1}`,meta:farm.location_name||'Mapped farm location',value:`${number(polygonStats(poly).area_hectares,2)} ha`,onClick:()=>setSection('area')}));
  const statusSource=[{id:`${farm.id}-status`,title:farm.status||'Unmapped',meta:`${farm.name} • Farm Boundary status`,value:'Farm',onClick:()=>setSection('status')}];
  ...
  return <>
    ...
    <section className="panel statistics-drilldown-panel">
      ...
      <div className="statistics-map-grid">
        <MapWorkspace farms={[farm]} sensors={farmSensors} plots={farmPlots} droneMappings={farmDrone} activeFarmId={farm.id} embedded modeHint={modeHint} height={500} selectedSensorId={selectedSensorId} selectedPlotId={selectedPlotId} selectedDroneId={selectedDroneId} focusTarget={focusTarget} onSensorClick={onSensor} onPlotClick={onPlot} onDroneClick={onDrone} showMapPopups={false} preview={preview} dataRevision={mapRevision}/>
        <div className="statistics-source-list section-transition-in" key={`${farm.id}-${section}`}>
          {activeNutrient && (farmSensors.length?farmSensors.map(s=><button type="button" key={s.id} className={selectedSensorId===s.id?'active':''} onClick={()=>onSensor?.(s)}><div><b>{s.sensor_code}</b><span>Sensor pin • {Number(s.latitude).toFixed(6)}, {Number(s.longitude).toFixed(6)}</span></div><strong>{number(s[activeNutrient.key],activeNutrient.digits)}{activeNutrient.suffix}</strong></button>):<Empty text="No sensor pins contribute to this statistic"/>)}
          {section==='sensors' && (farmSensors.length?farmSensors.map(s=><button type="button" key={s.id} className={selectedSensorId===s.id?'active':''} onClick={()=>onSensor?.(s)}><div><b>{s.sensor_code}</b><span>{s.status||'Online'} • {Number(s.latitude).toFixed(6)}, {Number(s.longitude).toFixed(6)}</span></div><strong>{s.coverage_m||50}m</strong></button>):<Empty text="No sensors on this farm"/>)}
          {section==='plots' && (farmPlots.length?farmPlots.map(p=><button type="button" key={p.id} className={selectedPlotId===p.id?'active':''} onClick={()=>onPlot?.(p)}><div><b>{p.plot_code}</b><span>{p.classification||'Pending'} • {niceDate(p.analyzed_at)}</span></div><strong>pH {number(p.ph,2)}</strong></button>):<Empty text="No soil-analysis plots on this farm"/>)}
          {section==='drone' && (farmDrone.length?farmDrone.map(d=><button type="button" key={d.id} className={selectedDroneId===d.id?'active':''} onClick={()=>onDrone?.(d)}><div><b>{d.name}</b><span>{d.classification||d.status||'Mapped'} • {niceDate(d.captured_at)}</span></div><strong>{number(d.area_hectares,2)} ha</strong></button>):<Empty text="No drone mapping data on this farm"/>)}
...
function SpatialRequestReviewCard({request,busy,onApprove,onReject,onClose}){
  ...
  return <aside className={`spatial-request-review-card status-${status}`}>
    ...
    {status==='pending'?<div className="request-review-actions"><button type="button" className="request-reject-btn" disabled={busy} onClick={onReject}><XCircle size={15}/>Reject</button><button type="button" className="primary-btn" disabled={busy} onClick={onApprove}><Check size={15}/>{busy?'Saving…':'Approve request'}</button></div>:<div className="request-reviewed-state"><Check size={15}/><span>This request is {status}. The decision was sent back to the Farmer.</span></div>}
  </aside>;
}
