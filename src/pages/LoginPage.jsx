...
export default function LoginPage() {
  const { user, role, login, demoLogin, authNotice, clearAuthNotice } = useAuth();
  ...
  const submit=async(e)=>{e.preventDefault();setBusy(true);setError('');clearAuthNotice?.();try{const next=await login(email,password);nav(next.labels?.includes('admin')?'/admin':'/farm',{replace:true});}catch(err){setError(err?.message||'Unable to sign in.');}finally{setBusy(false)}};
