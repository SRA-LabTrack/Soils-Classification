...
export async function subscribeFarmChanges(farmId, onChange) {
  ...
  // Spatial journal events remain the normal low-volume sync trigger. Farmer
  // approval decisions also update spatial_requests, so subscribe to that one
  // farm-scoped channel as a second tiny trigger. This lets v1.10.31 return the
  // Admin approval response immediately without waiting for a secondary journal
  // write, while the Farmer still reloads the authoritative farm bundle at once.
  ...
  const subscriptions=[];
  const errors=[];
  for(const channel of [changeChannel,requestChannel]){
    try{subscriptions.push(await realtime.subscribe(channel,onChange,[Query.equal('farm_id',[farmId])]));}
    catch(error){errors.push(error);}
  }
  if(!subscriptions.length)throw errors[0]||new Error('Farm Realtime could not connect.');
...
export async function subscribeSupportChanges(farmerId, onChange, {admin=false}={}) {
  ...
  if(admin){
    ...
    for(const channel of [supportChannel,requestChannel]){
      try{subscriptions.push(await realtime.subscribe(channel,onChange,queries));}
      catch(error){errors.push(error);}
    }
    if(!subscriptions.length)throw errors[0]||new Error('Support Realtime could not connect.');
  }
